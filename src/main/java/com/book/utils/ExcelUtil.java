...
public class ExcelUtil {
      ...
      try{
         HSSFWorkbook workbook = new HSSFWorkbook( input );
          ...
          if (!sheetName.trim().equals( "" )){
              //如果指定sheet名，则取指定sheet 中的内容
              sheet = workbook.getSheet( sheetName );
          }
          if(sheet == null){
              //如果传入的sheet 名不存在则默认指向第1个sheet
    ...
    /**
     * @param lists 对list数据源将其里面的数据导入到excel表单
     * @param sheetNames
     * @param output
     * @return
     */
        ...
        for (int ii = 0; ii < lists.length; ii++) {
            ...
            for (Field field : fields) {
                ...
                if (!attr.prompt().trim().equals("")) {
                    // 这里默认设了2-101列提示.
                    setHSSFPrompt(sheet, "", attr.prompt(), 1, lists[0].size(), col, col);
                }
                // 如果设置了combo属性则本列只能选择不能输入
                if (attr.combo().length > 0) {
                    // 这里默认设了2-101列只能选择不能输入.
                    setHSSFValidation(sheet, attr.combo(), 1, lists[0].size(), col, col);
                }
